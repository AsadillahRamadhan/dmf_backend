...
import MagnetArrayRepository from "App/Repositories/MagnetArrayRepository";

@inject()
export default class MagnetArrrayService {
    constructor(protected repository: MagnetArrayRepository){}
    ...
    public async destroy(id: number){
        try {
            ...
            const magnetRelation = await this.repository.checkMagnet(id);

            if(!magnetRelation){
                return {
                    success: false,
                    message: 'There are magnet(s) linked!',
                    code: 404
                }
            }
